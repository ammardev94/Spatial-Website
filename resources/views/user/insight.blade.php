...
<div class="page-topbar d-md-flex d-block align-items-center justify-content-between ">
    <div class="left-column my-auto mb-2">
        ...
        <h3 class="page-title mb-0">Insights</h3>
    </div>
...
<div class="listing-main">
    <div class="row g-0">
        ...
        <div class="col-md-12">
            <a href="{{ route('user.insight.show', [$featuredInsight->id]) }}" class="card featured-box mainbox">
                <div class="card-body">
                    <h6 class="card-title">{{ $featuredInsight->title }}</h6>
                    <p class="card-text">
                        {{ \Illuminate\Support\Str::limit(strip_tags($featuredInsight->description), 700) }} 
                    </p>
                    <p class="card-text created-at text-muted mb-0">
                        {{ $featuredInsight->created_at->diffForHumans() }}
                    </p>
                </div>
</a>
        </div>
        @endempty
        <div class="row g-0" id="insight-container">
            @include('user.partials.insight-list', ['insights' => $insights])
        </div>
        <div class="col-md-12 d-flex justify-content-center mt-4">
            @if ($insights->hasMorePages())
            <button class="btn btn-primary btn-lg" id="load-more"
                data-next-page="{{ $insights->currentPage() + 1 }}">
...
<script>
    $(document).ready(function() {
        ...
        const insightRoute = "{{ route('user.insight') }}";
        ...
        $('#load-more').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#insight-container').append(response);
